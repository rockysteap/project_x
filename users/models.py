...
#         return super().get_queryset().filter(is_active=True, type=User.Types.STAFF)
#
#
# class ActiveStudentManager(models.Manager):
#     def get_queryset(self):
#         return super().get_queryset().filter(is_active=True, type=User.Types.STUDENT)
...
class User(AbstractUser):
    ...
    type = models.CharField(max_length=7, choices=Types.choices, default=Types.PARENT, verbose_name='Тип записи')
    ...
    # objects = UserManager()
    # staff = ActiveStaffManager()
    # students = ActiveStudentManager()
